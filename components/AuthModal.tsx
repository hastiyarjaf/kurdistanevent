...
import { GoogleIcon, FacebookIcon, ChevronLeftIcon } from './icons';
...
export const AuthModal: React.FC<AuthModalProps> = ({ isOpen, onClose, onLogin, onSignUp, initialMode, onForgotPassword, lang }) => {
  ...
  const t = {
    // New
    welcome: { en: 'Welcome to Iraq Events', ar: 'أهلاً بك في فعاليات العراق', ku: 'بەخێربێیت بۆ ڕووداوەکانی عێراق' },
    joinPrompt: { en: 'Join or sign in to continue', ar: 'انضم أو سجل الدخول للمتابعة', ku: 'بەشداربە یان بچۆ ژوورەوە بۆ بەردەوامبوون' },
    continueWithGoogle: { en: 'Continue with Google', ar: 'المتابعة بحساب جوجل', ku: 'بەردەوامبوون بە گوگڵ' },
    continueWithFacebook: { en: 'Continue with Facebook', ar: 'المتابعة بحساب فيسبوك', ku: 'بەردەوامبوون بە فەیسبووک' },
    continueWithEmail: { en: 'Continue with Email', ar: 'المتابعة بالبريد الإلكتروني', ku: 'بەردەوامبوون بە ئیمەیڵ' },
  ...
  const renderSocialStep = () => (
    <>
        <div className="flex justify-between items-center mb-2">
            <h2 className="text-2xl font-bold text-gray-100">{t.welcome[lang]}</h2>
            <button onClick={onClose} className="text-gray-500 hover:text-gray-300 text-3xl leading-none">&times;</button>
        </div>
        <p className="text-gray-400 mb-6">{t.joinPrompt[lang]}</p>
        ...
        <div className="flex flex-col gap-3">
            <button onClick={() => onLogin('google')} className="w-full flex items-center justify-center gap-3 px-4 py-2 border border-gray-600 rounded-md hover:bg-gray-700">
                ...
                <span className="text-sm font-medium text-gray-200">{t.continueWithGoogle[lang]}</span>
            </button>
            <button onClick={() => onLogin('facebook')} className="w-full flex items-center justify-center gap-3 px-4 py-2 bg-[#1877F2] text-white rounded-md hover:bg-[#166eab]">
                ...
                <span className="text-sm font-medium">{t.continueWithFacebook[lang]}</span>
            </button>
        </div>
        ...
        <button onClick={() => setStep('email')} className="w-full px-4 py-2 bg-gray-600 text-white rounded-md hover:bg-gray-500">
    ...
    <>
      ...
      <form onSubmit={handleEmailSubmit} className="space-y-4">
        ...
        <div>
          <div className="flex justify-between items-center">
            ...
            {mode === 'login' && (
              <button type="button" onClick={() => setMode('forgot-password')} className="text-xs font-medium text-orange-400 hover:underline focus:outline-none">
        ...
        <button type="submit" className="w-full px-4 py-2 bg-orange-600 text-white rounded-md hover:bg-orange-700">
      ...
      <div className="mt-4 text-center">
        <p className="text-sm text-gray-400">
          ...
          <button onClick={() => switchMode(mode === 'login' ? 'signup' : 'login')} className="font-medium text-orange-400 hover:underline ltr:ml-1 rtl:mr-1">
    ...
    <>
      ...
      {resetSent ? (
        <div className="text-center py-4">
          ...
          <button onClick={() => { setMode('login'); setStep('email'); }} className="mt-6 w-full px-4 py-2 bg-orange-600 text-white rounded-md hover:bg-orange-700">
              {t.backToSignIn[lang]}
          </button>
        </div>
      ) : (
        <form onSubmit={handleForgotPasswordSubmit} className="space-y-4">
          ...
          <div className="text-center pt-2">
              <button type="button" onClick={() => { setMode('login'); setStep('email'); }} className="font-medium text-orange-400 hover:underline">
                  {t.backToSignIn[lang]}
              </button>
          </div>
        </form>
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-70 flex justify-center items-center z-50 p-4">
      <div className="bg-gray-800 rounded-lg shadow-xl w-full max-w-md p-6 modal-animate">
        {mode === 'forgot-password' ? renderForgotPassword() : (step === 'social' ? renderSocialStep() : renderEmailStep())}
      </div>
    </div>
  );
};