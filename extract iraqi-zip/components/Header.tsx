...
export const Header: React.FC<HeaderProps> = ({ lang, onLangChange, onOpenCreateModal, onOpenAIAssistant, currentUser, onAuthClick, onLogout, onOpenCurrentUserProfile, onToggleMyEvents, isMyEventsActive }) => {
  const t = {
      ...
      <div className="container mx-auto px-4">
        ...
        <div className="flex justify-between items-center">
          ...
          <div className="flex items-center gap-3">
            <LogoIcon className="h-10 w-10 text-orange-400" />
            ...
            {currentUser ? (
              <>
                <UserMenu 
                    user={currentUser} 
                    onLogout={onLogout} 
                    onOpenProfile={onOpenCurrentUserProfile} 
                    onToggleMyEvents={onToggleMyEvents}
                    isMyEventsActive={isMyEventsActive}
                    lang={lang}
                />
                <button 
                ...
                <button 
                  ...
                  className="text-sm font-semibold text-white bg-orange-500 rounded-full px-4 py-2 hover:bg-orange-600 transition-colors shadow-lg hover:shadow-xl transform hover:scale-105"
                >
                  {t.createEvent[lang]}
                </button>
              </>
            ) : (
              <button onClick={() => onAuthClick('login')} className="text-sm font-semibold text-gray-900 bg-orange-500 border border-orange-500 rounded-full px-6 py-2 hover:bg-orange-600 transition-colors">
                  {t.login[lang]}
                </button>
            )}
          </div>
